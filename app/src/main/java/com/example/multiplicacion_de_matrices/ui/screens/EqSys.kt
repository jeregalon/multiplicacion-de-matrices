...
import com.example.multiplicacion_de_matrices.components.EqSysInput
...
fun EqSys(
    ...
    val coefficientsMatrix = remember { MatrixState(3, 3) }
    val rhsMatrix = remember { MatrixState(1, 3) }
    var solution by remember { mutableStateOf<List<List<Double>>?>(null) }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Sistema de ecuaciones") },
        ...
        Column(
            ...
            Row(
                ...
                InterfaceButton(
                    onClick = { if (coefficientsMatrix.sizeX < 5) coefficientsMatrix.sizeX++; coefficientsMatrix.sizeY++; rhsMatrix.sizeY++ },
                    enabled = coefficientsMatrix.sizeX < 5,
                    textValue = "Agregar ecuación",
                    modifier = Modifier
                        .weight(1f),
                    fontSize = 13.sp
                )
                InterfaceButton(
                    onClick = { if (coefficientsMatrix.sizeX > 1) coefficientsMatrix.sizeX--; coefficientsMatrix.sizeY--; rhsMatrix.sizeY-- },
                    enabled = coefficientsMatrix.sizeX > 1,
                    textValue = "Quitar ecuación",
                    modifier = Modifier
                        .weight(1f),
                    fontSize = 13.sp
                )
            ...
            // Botón de Resolver Sistema

            InterfaceButton(
                onClick = {
                    try {
                        solution = MatrixOperations.multiplyMatrixes(MatrixOperations.calculateInverse(coefficientsMatrix.toDoubleMatrix()), rhsMatrix.toDoubleMatrix())
                        errorMessage = null
                    } catch (e: Exception) {
                        errorMessage = when (e) {
                ...
                Text(
                    text = buildString {
                        for (i in 0 until matrix.size) {
                            append(variables[i] + " = " + "%.4f".format(matrix[i][0]))
                            append("\n")
                        }
                    }
                )
